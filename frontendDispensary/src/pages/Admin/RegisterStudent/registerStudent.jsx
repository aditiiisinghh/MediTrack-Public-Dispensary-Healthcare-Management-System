...
const RegisterStudent = (props) => {
    ...
    const openCloseModal = () => { setReportModal(prev => !prev); }

    const handleOnChange = (value) => { setSearchStudent(value); }
    ...
    const handleSearch = async () => {
        ...
        props.showLoader();
        await axios.get(`http://localhost:4000/api/auth/get-student/${searchStudent}`, { withCredentials: true }).then(resp => {
            setStudentDetail(resp.data.student);
            toast.success("Student found!");
        }).catch(err => {
            toast.error(err?.response?.data?.error || "Student not found");
            setStudentDetail({ _id: "", email: "", name: "", roll: searchStudent, mobileNo: "", fatherName: "", fatherMobile: "", address: "", previous_health: "", age: "", bloodGroup: "" });
        }).finally(() => {
            props.hideLoader();
        });
    }
    ...
    const registerStudent = async () => {
        ...
        props.showLoader();
        await axios.post('http://localhost:4000/api/auth/register-student', studentDetail, { withCredentials: true }).then(resp => {
            toast.success("Student Registered! Password sent to email.");
                ...
                <form className='register-form' onSubmit={handleSubmit}>
                    <div className='register-form-div'>
                        <div className='register-input-box'>
                            <input value={studentDetail.name} onChange={(event) => handleOnChangeInputField(event, "name")} className='input-box-register' placeholder='Full Name' type='text' />
                        </div>
                        <div className='register-input-box'>
                        ...
                        <div className='register-input-box'>
                            <input value={studentDetail.fatherMobile} onChange={(event) => handleOnChangeInputField(event, "fatherMobile")} className='input-box-register' placeholder='Father Mobile' type='text' />
                        </div>
                        <div className='register-input-box'>
