...
exports.addNearByHospital = async (req, res) => {
    try {
        let body = { ...req.body };
        const hospital = new HospitalModels({ ...body, addedBy: req.user._id });
        await hospital.save();
...
exports.getHospitals = async (req, res) => {
    try {
        const hospitals = await HospitalModels.find().sort({ createdAt: -1 });
...
exports.updateHospitalById = async (req, res) => {
    try {
        const { id } = req.params;
        let body = { ...req.body };
        const hospital = await HospitalModels.findByIdAndUpdate(id, { ...body }, { new: true });
        if (hospital) {
